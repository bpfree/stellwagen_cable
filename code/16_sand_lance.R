#######################################
### 16. critical sand lance habitat ###
#######################################
...
### input directories
#### sand lance
data_dir <- "data/a_raw_data/Sand_Lance_FGDB.gdb"
...
data <- sf::st_read(dsn = data_dir,
                    layer = sf::st_layers(dsn = data_dir)[[1]][grep(pattern = stringr::str_glue("{data_name}"),
...
data_region_grid <- grid[data_region, ] %>%
  # spatially join sand lance to Stellwagen grid
  sf::st_join(x = .,
              y = data_region,
              join = st_intersects) %>%
