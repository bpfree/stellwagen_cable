...
barriers_without_coral_boulder <- terra::rast(file.path(raster_dir, stringr::str_glue("{region_name}_barriers_without_coral_boulder_{cell_size}m.grd")))
...
cost_rm_barriers_without_coral_boulder <- c(cost_raster,
                                            barriers_without_coral_boulder) %>% 
  # sum the two layers while removing any NA values
  terra::app(sum, na.rm = T) %>%
  # add 0.01 so there are no 0 values
  sum(., 0.01)
plot(cost_rm_barriers_without_coral_boulder)

# make any values above 99 (where a constraint would be) to be set as NA to remove from analysis
cost_rm_barriers_without_coral_boulder[cost_rm_barriers_without_coral_boulder >= 99] <- NA
cost_rm_barriers_without_coral_boulder[cost_rm_barriers_without_coral_boulder == 0.01] <- NA
plot(cost_rm_barriers_without_coral_boulder)
...
terra::writeRaster(cost_rm_barriers_without_coral_boulder, filename = file.path(raster_dir, stringr::str_glue("{region_name}_costs_rm_barriers_without_coral_boulder_{cell_size}m.grd")), overwrite = T)
